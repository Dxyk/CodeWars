...
an error (return Nothing in Haskell, return None in F# and Ocaml, return -1 in
C) so that he clearly sees that his expectations are not great enough.
...
new_avg([14, 30, 5, 7, 9, 11, 15], 2) should raise an error (ValueError or
invalid_argument)
 */

long long newAvg(double *arr, size_t szArray, double navg) {
	double exp_total = navg * (szArray + 1);
	double current_total = 0;
	for (int i = 0; i < szArray; i++) {
		current_total += arr[i];
	}
	double diff = exp_total - current_total;
	if (diff < 0) {
		return -1;
	}
	return ((int)(diff * 10) % 10 > 5) ? diff + 1 : diff;
}

int main(int argc, char const *argv[]) {
	double arr[7] = {14, 30, 5, 7, 9, 11, 15};
	printf("%llu\n", newAvg(arr, 7, 92));
	return 0;
}
